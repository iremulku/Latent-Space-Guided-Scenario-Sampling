...
class LoRA_ViT(nn.Module):
    ...
    def __init__(self, vit_model: ViT, r: int, num_classes: int = 0, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(vit_model.transformer.blocks):
            ...
            # w_q_linear.weight.requires_grad = False
            # w_q_linear.bias.requires_grad = False
        ...
        # print("Gradients of w_q_linear", w_q_linear.weight.grad)

        # print("Gradients of w_a_linear_q", w_a_linear_q.weight.grad)
        # print("Gradients of w_b_linear_q", w_b_linear_q.weight.grad)
...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 1, lora_layer=None):
        super(LoRA_ViT_timm, self).__init__()

        assert r > 0
        if lora_layer:
            self.lora_layer = lora_layer
            #self.lora_layer = vit_model.b3
        else:
