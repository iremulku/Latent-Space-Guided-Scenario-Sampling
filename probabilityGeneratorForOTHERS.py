...
modelPath = r"C:\Users\Public\Server\experiments\LORA_MULTIMODAL_OLDS\DSTL\Latentfactorizaton\2025_12_8_9_54_model0"
CHECKPOINT_PATH = os.path.join(modelPath, modelName)
OUTPUT_JSON = os.path.join(modelPath, "dstl_scenario_probabilities.json")

# Only shared term is used
alpha = 1.0
tau = 0.5
gamma_floor = 0.5
lam = 1e-3
...
class LatentHookCollector:
    """
    Shared-only hook collector for the original MMVit4/MMVit5 architecture.

    We hook the output of `multimodal_decode_conv`, i.e. x6_inter,
    which is the final shared multimodal latent representation before decoding.
    """
    def __init__(self, model):
        ...
        self.handles.append(
            model.multimodal_decode_conv.register_forward_hook(
...
def compute_scenario_statistics(model, data_loader):
        ...
        for images, _ in data_loader:
            ...
            if "z_shared" not in hooks.cache:
                raise RuntimeError(
                    ...
                    "Check whether multimodal_decode_conv is the correct hook point."
                )
    ...
    # Since this version is shared-only, the scenario descriptor is 1D.
    X = D_sh[:, None]
...
def main():
    ...
    print("Building original MMVit4/MMVit5 model...")
    model = MMVit4().to(device)   # change to MMVit4() if your local class is MMVit4
    model.load_state_dict(torch.load(CHECKPOINT_PATH, map_location=device))
    model.eval()

    print("Computing shared-only scenario statistics...")
